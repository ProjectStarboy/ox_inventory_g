...
function server.setPlayerInventory(player, data)
	...
	local inv = Inventory.Create(player.source, player.name, 'player', shared.playerslots, totalWeight, shared
...
lib.addCommand('removeitem', {
	...
	if item and args.count > 0 then
		...
		if not success then
			return Citizen.Trace(('Failed to remove %sx %s from player %s (%s)'):format(args.count, item.name, args.target,
				response))
		end
...
lib.addCommand('setitem', {
	...
	if item then
		...
		if not success then
			return Citizen.Trace(('Failed to set %s count to %sx for player %s (%s)'):format(item.name, args.count, args
				.target, response))
		end
